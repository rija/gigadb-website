...
docker-compose run --rm uploader curl -L -O https://github.com/kencho51/ExceltoGigaDB/archive/refs/heads/update-jdbc-driver.zip

# Unpack source files in place
# docker-compose run --rm uploader bsdtar -k --strip-components=1 -xvf develop.zip
docker-compose run --rm uploader bsdtar -k --strip-components=1 -xvf update-jdbc-driver.zip

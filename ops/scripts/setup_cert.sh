...
if [[ $cert_files_local_exists == 'true' ]];then
  ...
  if [[ $fullchain_pem_remote_exists == "true" && $privkey_pem_remote_exists == "true" && $chain_pem_remote_exists == "true" ]];then
    ...
    $DOCKER_COMPOSE run --rm config bash -c "echo $remote_fullchain > $FULLCHAIN_PEM"
    ...
    remote_privkey=$($DOCKER_COMPOSE run --rm config bash -c "/usr/bin/curl --show-error --silent \
      ...
      --header 'PRIVATE-TOKEN: $GITLAB_PRIVATE_TOKEN' | cat | jq -r '.value'")
    $DOCKER_COMPOSE run --rm config bash -c "echo $remote_privkey > $PRIVATE_PEM"
    ...
    remote_chain=$($DOCKER_COMPOSE run --rm config bash -c "/usr/bin/curl --show-error --silent \
      ...
      --header 'PRIVATE-TOKEN: $GITLAB_PRIVATE_TOKEN' | cat | jq -r '.value'")
    $DOCKER_COMPOSE run --rm config bash -c "echo $remote_chain > $CHAIN_PEM"

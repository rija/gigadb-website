...
if ! [ -s ./.secrets ];then
    ...
    curl -s --header "PRIVATE-TOKEN: $GITLAB_PRIVATE_TOKEN" "${MISC_VARIABLES_URL}?per_page=100" | jq --arg ENVIRONMENT $REVIEW_ENV -r '.[] | select(.environment_scope == "*" ) | select(.key | test("sftp_") ) | .key + "=" + .value' > .misc_var
